...
jni::Global<jni::Object<Types::WorldObject>> createSomeRect(jni::JNIEnv& env, Self& self) {
  auto* runner = Handles::get<NovelRunner>(env, *self);
  ...
  auto worldObject = jni::NewGlobal(env, Classes->WorldObject.New(env, Constructors->WorldObject_main,
                                              jni::jlong(rectPtr), jni::jboolean(true)));

  runner->SceneConstructionRequested += [rectPtr]() {
    rectPtr->executeObjectBehaviour();
  };

  return worldObject;
}

jni::Local<jni::Object<Types::SceneConstructionRequestedEvent>> createSceneConstructionRequestedEvent(jni::JNIEnv& env, Self& self) {
  auto* runner = Handles::get<NovelRunner>(env, *self);
  auto* event = &runner->SceneConstructionRequested;
  std::cout << "Creating SceneConstructionRequestedEvent for event @ " << event << std::endl;
...
void Bindings::registerNovelRunnerBindings(jni::JNIEnv& env) {
  jni::RegisterNatives(env, *Classes->NovelRunner,
                       ...
                       jni::MakeNativeMethod<decltype(createSomeRect), &createSomeRect>("createSomeRect"),
                               jni::MakeNativeMethod<decltype(createSceneConstructionRequestedEvent), &createSceneConstructionRequestedEvent>("createSceneConstructionRequestedEvent")
  );
}
