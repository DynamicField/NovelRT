...
class FinalFileWriter(
  ...
  fun write(types: List<DefinedGenerationType> = codeGenerationModel.getDefinedTypesByReferenceOrder()) {
    cppWriter.beginIncludeGuard(generateIncludeGuardName())
    cppWriter.writeLine()
  ...
  private fun generateIncludeGuardName(): String {
    val uniqueId = UUID.randomUUID().toString().replace('-', '_').toUpperCase()
    return "NOVELRT_GENERATED_TYPES_${uniqueId}"
  }
